...
def train_test_split(X, y, train_fraction=0.8):
    n = X.shape[0] # Take the # of examples as n
    shuffled = torch.randperm(n) # Creates a random permutation/order of the integers from range(0, n); e.g. [1, 0, 3, 2, 4] for n = 5
    train_size = int(n * train_fraction) # Number of examples to include in the training set (80% by default)
    train_idx = shuffled[:train_size] # Takes the first train_size shuffled indices e.g. [1, 0, 3, 2]
    test_idx = shuffled[train_size:] # Takes the remainder train_size shuffled indices e.g. [4]
